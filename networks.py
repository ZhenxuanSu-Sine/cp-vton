...
def KL_to_normal(q_dist):
    b, c, w, h = q_dist.shape
    assert (c % 2 == 0)
    K = c // 2
    mu_q = q_dist[:, :K]
...
class GMM(nn.Module):
    ...
    def gaussian_noise(self, shape):
        return torch.normal(torch.zeros(shape), torch.ones(shape)).cuda()

    def gauss_sampling(self, feature):
